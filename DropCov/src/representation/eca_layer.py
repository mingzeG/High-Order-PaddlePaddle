...
class eca_layer_drop(nn.Layer):
    ...
    def dy_drop(self, y):
        b, c,_,_ = y.size()       
        # mask = paddle.zeros(size = y.shape) # cpu
        mask = paddle.zeros(size = y.shape).cuda() # gpu
        sort_sum, index = paddle.sort(y, dim=1, descending= True)
