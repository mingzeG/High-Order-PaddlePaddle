...
class ShiftModule(nn.Module):
    ...
    def __init__(self, input_channels, n_segment=16, n_div=8, mode='shift'):
...
class Bottle2neckShift(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26, scale=4, stype='normal'):
        ...
        self.me = MEModule(width*scale, reduction=16, n_segment=16)
        ...
        for i in range(self.nums):
            ...
            shifts.append(ShiftModule(width, n_segment=16, n_div=2, mode='fixed'))
        shifts.append(ShiftModule(width, n_segment=16, n_div=2, mode='shift'))
    ...
    def forward(self, x):
        ...
        elif self.scale != 1 and self.stype == 'stage': #gzl 05.20
...
def tea50_16f(TCP_module=None, **kwargs):
...
if __name__ == '__main__':
    ...
    model = tea50_16f(pretrained=True)
    output = model(images)
    print(output.size())