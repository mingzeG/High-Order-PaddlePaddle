...
class TSA(nn.Layer):
    ...
    def forward(self, x):
        ...
        V = N // T
        x = x.view(V, T, C, H, W)
        x_1 = paddle.cat((x[:,0,:,:,:].unsqueeze(1),x[:,:-1,:,:,:]), 1)
        x_2 = paddle.cat((x_1[:, 0, :, :, :].unsqueeze(1), x_1[:, :-1, :, :, :]), 1)
        ...
        theta_x_1_2 = paddle.einsum("nca,ncb->nab", (phi_x_1, phi_x_2))
        ...
        x_out = paddle.einsum("ntg,ncg->nct", (theta_x_1_2, phi_x0))
