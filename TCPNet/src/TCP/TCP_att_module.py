...
from TCP.TSA import TSA
from TCP.TCA import TCA

class TCP_att_module(nn.Layer):
    ...
    def _construct_TCP_att(self
        ...
        if self.conv_1d_flag :
            ...
            if (paddle.__version__).split('.')[1] >= '5': #version 1.5+
    ...
    def init_weights(self):
            for m in self.named_parameters():
                if isinstance(m, nn.Conv2DBatchNorm2D) or isinstance(m, nn.Conv3D):
                    #c2_msra_fill(m)
                    m.weight.data.normal_(mean=0.0, std=0.01)
                    if m.bias is not None:
                        m.bias.data.zero_()
                elif isinstance(m, nn.BatchNorm2D) \
                        or isinstance(m, nn.BatchNorm3DConv3D):
                    batchnorm_weight = 0.
