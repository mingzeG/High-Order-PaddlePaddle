...
class TCP(nn.Layer):
     def __init__(self, dim_in=2048, dim_out=128, num_segments=8, level='video',
         ...
         if self.sp_flag or self.ch_flag or self.conv_1d_flag:
             from .TCP_att_module import TCP_att_module as att_module
             self.TCP_att = att_module(
                 dim_out, frame=num_segments,
                 sp_flag=sp_flag, ch_flag=ch_flag, conv_1d_flag=conv_1d_flag)
         else :
             # w/o att Layer, only matrix normalization
             from Identity import Identity as att_module
             self.TCP_att = att_module()
     ...
     def parameter_init(self):
         for n, m in self.named_parameters():
             if m == self.TCP_att :
                 # print('-----skip att Layer initialization in TCP Layer---')
                 return 0
