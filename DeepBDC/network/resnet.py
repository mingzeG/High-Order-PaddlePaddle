...
class SimpleBlock(nn.Layer):
    ...
    def __init__(self, indim, outdim, half_res):
        ...
        self.indim = indim
        self.outdim = outdim
        self.C1 = nn.Conv2D(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias_attr=False)
        self.BN1 = nn.BatchNorm2D(outdim)
        self.C2 = nn.Conv2D(outdim, outdim, kernel_size=3, padding=1, bias_attr=False)
        ...
        if indim != outdim:
            ...
            self.shortcut = nn.Conv2D(indim, outdim, 1, 2 if half_res else 1, bias_attr=False)
...
class BottleneckBlock(nn.Layer):
    ...
    def __init__(self, indim, outdim, half_res):
        ...
        self.indim = indim
        self.outdim = outdim
        self.C1 = nn.Conv2D(indim, bottleneckdim, kernel_size=1, bias_attr=False)
        self.BN1 = nn.BatchNorm2D(bottleneckdim)
        self.C2 = nn.Conv2D(bottleneckdim, bottleneckdim, kernel_size=3, stride=2 if half_res else 1, padding=1)
        self.BN2 = nn.BatchNorm2D(bottleneckdim)
        self.C3 = nn.Conv2D(bottleneckdim, outdim, kernel_size=1, bias_attr=False)
        ...
        # if the input number of channels is not equal to the output, then need a 1x1 convolution
        if indim != outdim:
            self.shortcut = nn.Conv2D(indim, outdim, 1, stride=2 if half_res else 1, bias_attr=False)
...
class DropBlock(nn.Layer):
    ...
    def _compute_block_mask(self, mask):
        ...
        offsets = paddle.stack(
            [
                paddle.arange(self.block_size).view(-1, 1).expand(self.block_size, self.block_size).reshape(-1), # - left_padding,
                paddle.arange(self.block_size).repeat(self.block_size), #- left_padding
            ]
        ).t().cuda()
        offsets = paddle.cat((paddle.zeros(self.block_size**2, 2).cuda().long(), offsets.long()), 1)
...
class BasicBlockVariant(nn.Layer):
    expansion = 1
