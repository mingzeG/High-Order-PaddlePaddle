...
class BaselineTrain(nn.Layer):
    ...
    def feature_forward(self, x):
        ...
        if self.params.method in ['stl_deepbdc', 'meta_deepbdc']:
            ...
            out = self.avgpool(out).reshape((out.shape[0], -1))
        return out
    ...
    def forward_meta_val(self, x):
        x = Variable(x.cuda())        
        x = x.reshape(self.params.val_n_way * (self.params.n_shot + self.params.n_query), *tuple(x.shape[2:]))
        ...
        z_proto = z_support.reshape((self.params.val_n_way, self.params.n_shot, -1)).mean(1)
        z_query = z_query.reshape((self.params.val_n_way * self.params.n_query, -1))
    ...
    def forward_meta_val_loss(self, x):
        y_query = paddle.from_numpy(np.repeat(range(self.params.val_n_way), self.params.n_query))
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            pred = output.max(1)[1]
            total_correct += pred.eq(y.expand_as(pred)).sum()
            loss.backward()
            optimizer.step()
...
class MetaTemplate(nn.Module):
    ...
    def parse_feature(self, x, is_feature):
        ...
        if is_feature:
            ...
            x = x.reshape(self.n_way * (self.n_support + self.n_query), *tuple(x.shape[2:]))
