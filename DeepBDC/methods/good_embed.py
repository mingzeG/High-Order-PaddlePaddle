...
class GoodEmbed(MetaTemplate):
    ...
    def feature_forward(self, x):
        out = self.avgpool(x).reshape((x.size(0),-1))
        return out
    ...
    def set_forward(self, x, is_feature=True):
        ...
        qry_norm = paddle.norm(z_query, p=2, dim=1).unsqueeze(1).expand_as(z_query)
        spt_norm = paddle.norm(z_support, p=2, dim=1).unsqueeze(1).expand_as(z_support)
